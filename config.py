...
반영. LIDAR_DETECT_HALF_WINDOW_DEG(각도창) 대신 LIDAR_LANE_WIDTH_MM
(실측 680mm, 차량 진행축 기준 좌우 폭 코리더)으로 판정 방식을 바꾸고,
AVOID_DURATION_SECONDS 하나였던 회피 기동을 AVOID_OUT/HOLD/RETURN_
SECONDS 세 구간으로 나눠 "피하고 다시 돌아오기"를 지원합니다. 자세한
설계 이유는 아래 "라이다 장애물 감지" / "장애물 회피" 섹션 주석과
obstacle_detector.py를 보세요. AVOID_HOLD_SECONDS는 아직 실차 튜닝 전
추정값이라 실제 장애물 폭/차 길이에 맞게 반드시 재검증해야 합니다.
"""
...
#
# LIDAR_LANE_WIDTH_MM은 실측한 2차선(우리 차선) 전체 폭입니다(680mm).
# 라이다가 차체 중앙에 고정되어 있다고 들어서(범퍼 아래 판을 덧대 그
# 앞에 장착, 좌우 오프셋 없음) 중앙선(0mm) 기준 좌우 대칭 ±340mm가
# 차선 경계입니다. 실측치가 바뀌거나 라이다가 중앙에서 벗어나 있으면
# 이 값과 아래 코드의 좌우 오프셋 가정을 함께 고쳐야 합니다. 이 값
# 자체는 판정에 직접 쓰이지 않고, 아래 LIDAR_OBSTACLE_LATERAL_*_MM을
# 정하기 위한 참고 치수입니다.
LIDAR_LANE_WIDTH_MM = 680.0

# [2026-08-06 Claude 수정] 처음엔 "내 차선 안이면 전부"(|lateral|<=340mm,
# 즉 위 LIDAR_LANE_WIDTH_MM의 절반 이내)로 판정했는데, 실제 장애물은
# 항상 차선 중앙선에서 150~230mm(15~23cm) 사이에만 놓인다는 걸 알게
# 됐습니다. 차선 중앙(0mm 근처)이나 차선 가장자리(340mm 근처)는 사람이
# 서 있거나 지나갈 수 있는 구간이라, 실제 장애물이 있을 좁은 구간으로만
# 시야를 좁혀서 사람을 장애물로 오검출하는 걸 막습니다. 부호(좌/우)는
# 안 가리고 |lateral|(중앙에서의 절대 횡방향 거리)로만 판정하므로,
# 장애물이 어느 쪽에 있어도 잡힙니다.
LIDAR_OBSTACLE_LATERAL_MIN_MM = 150.0
LIDAR_OBSTACLE_LATERAL_MAX_MM = 230.0
