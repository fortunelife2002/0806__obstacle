...
# 46cm->-50mm, 56cm->+50mm이므로 |lateral|<=50mm 구간만 봅니다. 부호
# (좌/우)는 가리지 않고 절대값으로 판정하므로, 차량이 살짝 안쪽/바깥쪽
# 으로 치우쳐 있어도(예: 라이다 중앙이 51cm에서 몇 mm 벗어나도) 크게
# 문제되지 않습니다. MIN은 지금은 0이라 사실상 안 쓰이지만, 나중에
# "장애물 바로 앞 몇 mm는 차체 자기간섭이라 제외" 같은 경우가 생기면
# 이 값을 올려서 대응할 수 있게 남겨뒀습니다.
...
# -------------------------------------------------------------------
# 장애물 회피(옆 차선 이동 후 복귀) 기동 (obstacle_detector.py: AvoidanceController)
# -------------------------------------------------------------------
# [2026-08-06 Claude 수정] "피하기만" 하던 기존 기동을 "피했다가 다시
# 원래 2차선으로 돌아온다"로 확장했습니다. 카메라 차선 인식은 "지금
# 보이는 오른쪽 선을 기준으로 삼는" 상대적 방식이라 1차선으로 넘어간
# 뒤 그냥 두면 1차선을 새 기준으로 착각하고 계속 눌러앉습니다. 그래서
# 복귀까지 아래 세 구간을 모두 오픈루프(카메라 무시)로 관리합니다.
#
#   OUT   : AVOID_STEER_OFFSET 방향으로 AVOID_OUT_SECONDS 동안 꺾어
#           1차선으로 넘어갑니다.
#   HOLD  : 조향을 중앙으로 되돌리고 AVOID_HOLD_SECONDS 동안 직진하며
#           장애물 옆을 실제로 통과합니다(장애물 폭+차량 길이만큼 지나갈
#           시간 — 라이다로 "지나쳤다"를 재확인하지 않는 순수 시간
#           기반이라 실차에서 가장 많이 튜닝해야 하는 값입니다).
#   RETURN: OUT과 반대 방향으로 AVOID_RETURN_SECONDS 동안 꺾어 다시
#           2차선으로 돌아옵니다. OUT과 대칭이라고 가정해 기본값은
#           AVOID_OUT_SECONDS와 같게 뒀지만, 좌우 서보/바퀴 특성이
#           다르면 따로 튜닝하세요.
#
# 이 세 구간 다음에 COOLDOWN을 거쳐 일반 차선 추종(2차선)으로 복귀합니다.
#
# [2026-08-06 Claude 수정] 35는 STEER_CENTER(157)에서 STEER_LEFT(210)
# 까지 전체 조향각의 약 66%(35/53)에 해당하는 꽤 급격한 각도였습니다.
# 실차에서 AVOID_OUT이 끝나는 시점에 카메라가 트랙 왼쪽 바깥을 보고
# 있을 정도로 헤딩이 과하게 돌아버린 걸 확인해서 20(약 38%)으로
# 낮췄습니다. 각도를 줄이면 같은 시간 동안 옆으로 이동하는 거리도
# 줄어드니, AVOID_OUT_SECONDS(현재 3.0초)를 다시 늘려야 할 수 있습니다
# — 각도와 시간을 같이 보면서 튜닝하세요.
AVOID_STEER_OFFSET = 20
...
# 1.4초(부족) -> 4.0초(과회전, 너무 왼쪽) -> 3.0초 순으로 좁혀오다가,
# AVOID_STEER_OFFSET도 35 -> 20으로 줄인 뒤 다시 2.0초로 시도합니다.
# 각도를 줄였으니 같은 시간에 옆으로 이동하는 거리는 줄었을 텐데도
# 시간을 오히려 더 줄이는 것이라, 1차선까지 확실히 안 들어갈 수 있으니
# 실차에서 꼭 재확인하세요. 이 값은 "얼마나 옆으로 이동했나"뿐 아니라
# "얼마나 회전했나"(헤딩)에도 그대로 비례합니다(같은 조향각을 그 시간
# 만큼 계속 유지). 실차에서 AVOID_HOLD 시작 시점에 차가 1차선에
# 헤딩까지 자연스럽게 들어가 있는지 계속 확인하세요.
AVOID_OUT_SECONDS = 2.0

# [2026-08-06 Claude 수정 2] AVOID_HOLD_SECONDS: 처음엔 "차량 110cm +
# 장애물 110cm를 다 지나가는 시간"으로 계산해 3.0초로 잡았는데, 1차선
# 에도 장애물이 있다는 걸 알게 되면서 이 계산 자체가 의미가 없어졌습니다
# (2차선 장애물 길이만 계산해서는 안 되고, 1차선 장애물과도 안 부딪히게
# 지나가야 함 — 아직 그 장애물의 정확한 위치/길이를 반영한 계산은 안
# 돼 있습니다). 우선 AVOID_OUT_SECONDS와 동일하게 4.0초로 맞춰두고
# 실차에서 재확인/재조정합니다.
#
# 주의: HOLD 구간에서는 라이다 감지를 아예 안 봅니다(AvoidanceController.
# update 참고) — 즉 지금은 1차선의 두 번째 장애물을 별도로 피하는 로직이
# 없고, 그냥 고정된 시간만큼 직진할 뿐입니다. 1차선 장애물 위치에 따라
# 이 구간에서 부딪힐 수 있으니, 실차 테스트 시 반드시 저속으로, 충돌
# 위험 없는 상태에서 확인하세요.
AVOID_HOLD_SECONDS = 4.0
