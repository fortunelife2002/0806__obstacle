...
# 직진을 시작하는 문제가 있어 4.0초로 늘렸다가, 이번엔 반대로 너무 왼쪽
# (조향 방향)으로 많이 돌아버려서 3.0초로 다시 낮췄습니다. 이 값은
# "얼마나 옆으로 이동했나"뿐 아니라 "얼마나 회전했나"(헤딩)에도 그대로
# 비례하는 값이라(같은 조향각을 그 시간만큼 계속 유지), 1.4(부족) ->
# 4.0(과함) -> 3.0 순으로 실차에서 좁혀가는 중입니다. 실차에서 AVOID_
# HOLD 시작 시점에 차가 1차선에 헤딩까지 자연스럽게 들어가 있는지 계속
# 확인하세요.
AVOID_OUT_SECONDS = 3.0
